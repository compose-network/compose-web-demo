...
const Sidebar = () => {
  ...
  return (
    <div className="flex flex-col h-full justify-between">
      <div className="h-full w-[264px] flex flex-col border-r-[1px] border-r-gray-300 p-2 gap-2">
        <NavLink to={"/"} className="p-4">
          <ComposeLogo className="h-[48px]" />
        </NavLink>
        <div>
          {ITEMS.map((item, index) => (<Text onClick={() => {
            if (!item.isFlowInProgress) {
              navigate(`/${item.route}`);
            }
          }} variant={"body-3-medium"}
                                             className={`w-full h-[48px] ${!item.isFlowInProgress && "cursor-pointer"} flex justify-between items-center p-4 rounded-[24px] ${(!item.isFlowInProgress && pathname.includes(item.route || "")) && "bg-primary-50 text-primary-500"}`}
                                             key={index}>
            {item.label}
      ...
      <div className="flex flex-col border-r-[1px] border-r-gray-300 p-2 gap-5 pb-5">
        ...
        <div className="w-full flex gap-4 ml-4">
          ...
          <Link target={"_blank"} to={"https://www.compose.network/"}><TbWorld
