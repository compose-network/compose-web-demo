...
import type { ComposeSmartAccount } from "@/lib/smart-account/kernel.ts";
...
export type GenerateERC20BridgeUserOpsParams = {
  eoaAddress: Address;
  sourceKernelAccount: ComposeSmartAccount;
  destKernelAccount: ComposeSmartAccount;
...
export const createAndSignBridgeERC20UserOps = async ({
  ...
  const { userOp: sourceUserOp } = await sourceKernelAccount.createUserOp([
    {
      to: tokenAddress,
      value: 0n,
  ...
  const { userOp: destUserOp } = await destKernelAccount.createUserOp([
    {
