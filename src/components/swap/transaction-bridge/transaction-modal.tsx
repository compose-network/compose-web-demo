        ...
        <div className="flex gap-2 flex-col">
          {data?.actions.map((action, i) => (
            ...
            >
              <div className="flex gap-4 p-5 bg-gray-100 items-center rounded-sm">
                ...
                <div className="flex flex-col gap-1">
                  <Text variant="body-2-medium" className="flex items-center gap-1">
                    {action.name}{" "}
                    {action.userOpData && (openData ? <ChevronUp onClick={onOpenDataHandler} className="h-4 w-4" /> : <ChevronDown onClick={onOpenDataHandler} className="h-4 w-4" />)}
                  </Text>
              ...
              {(openData) && action.userOpData?.map((userOp, i) => (
                <div
                  className="w-[498px] bg-white border border-gray-300 rounded-[8px] flex flex-col p-4 text-gray-600 gap-4 overflow-x-auto">
                  <div className={`flex gap-1 items-center text-gray-600 ${i === action.userOpData?.length && 'mb-5'}`}><ChainIcon size="sm"
                                                                                                                         chainId={userOp.chainId} /> <Text
                    className={"text-gray-600"} variant={"body-3-medium"}>{getChainById(userOp.chainId).name}</Text>
                  </div>
                  <div><Text variant="body-3-medium"
                             className="whitespace-pre-wrap font-mono text-xs">{JSON.stringify(JSON.parse(userOp.data), null, 2)}</Text>
                  </div>
                </div>
              ))}
            </div>
