...
export const Flashloans = () => {
  const testingEOA = "0x5b601de979c4d658632e5b528c6af42f05e2d296";
  ...
  // console.log(
  //   "Source 1: USDC -> WETH",
  //   formatEther(arbitrage.data?.optimalLoanAmount ?? 0n),
  //   "->",
  //   formatEther(quoteSourceChain.data?.[0] ?? 0n),
  // );
  // console.log(
  //   "Dest 2: WETH -> USDC",
  //   formatEther(quoteSourceChain.data?.[0] ?? 0n),
  ...
  const submit = async (ev: React.FormEvent<HTMLFormElement>) => {
    ...
    const flashAdapterContract =
      "0x96C08A7A6d0aE2BEc9bb816dBacEC39d30C1dFe1"; /* ||
    ...
    const [sendWETH_from_source, receiveWETH_on_dest] = createBridgeCalldata({
      ...
      token: WETH_ADDRESS,
      amount: quoteSourceChain.data?.[0] ?? 0n,
    ...
    const [sendUSDC_from_dest, receiveUSDC_on_source] = createBridgeCalldata({
      ...
      token: USDC_ADDRESS,
      amount: quoteDestChain.data?.[0] ?? 0n,
      sessionId: BigInt(Math.floor(Math.random() * 1000000)),
    });
    ...
    ops.forEach((op) => {
      ...
      op.userOp.callGasLimit = (op.userOp.callGasLimit * mult) / 10n;
      op.userOp.maxFeePerGas = (op.userOp.maxFeePerGas * mult) / 10n;
      op.userOp.maxPriorityFeePerGas =
        (op.userOp.maxPriorityFeePerGas * mult) / 10n;
      op.userOp.preVerificationGas =
        (op.userOp.preVerificationGas * mult) / 10n;
      op.userOp.verificationGasLimit =
        (op.userOp.verificationGasLimit * mult) / 10n;
      console.log("op:", op.userOp.callGasLimit);
    });
  ...
  const usdc = useBalanceOf(
    ...
    {
      account: testingEOA as `0x${string}`,
    },
  );
